...
function SVGPathTracing() {
  const [outerPathLength, setOuterPathLength] = React.useState(0);
  const [innerPathLength, setInnerPathLength] = React.useState(0);
  const [circlePathLength, setCirclePathLength] = React.useState(0);

  React.useLayoutEffect(() => {
    const outerPath = document.querySelector("#outerPath");
    const innerPath = document.querySelector("#innerPath");
    const circlePath = document.querySelector("#circlePath");
    setOuterPathLength(outerPath.getTotalLength() || 0);
    setInnerPathLength(innerPath.getTotalLength() || 0);
    setCirclePathLength(circlePath.getTotalLength() || 0);
  }, []);
  ...
  const [outerPathProps] = useFriction({
    from: {
  ...
  const [innerPathProps] = useFriction({
    from: {
      strokeDashoffset: innerPathLength
    },
    to,
    config,
    infinite: true
  ...
  const [circlePathProps] = useFriction({
    from: {
  ...
  return (
    <div className="live-preview__center">
      <svg
        ...
        strokeLinecap="round"
        style={{ height: ${context === 'gallery-preview' ? 150 : 230}}}
      >
        <path
        ...
        <path
          ...
          className="live-preview__stroke"
          id="outerPath"
