...
| reducedMotion               | `CSSPairs?`                        | **Optional.** The animation configuration to use if the end user prefers reduced motion. If unspecified and the end user prefers reduced motion, the animating element will be set immediately to the `to` state to avoid potentially harmful animation.          |
...
| onFrame                     | `?(number, MotionVectors) => void` | **Optional.** Supply a callback function that `renature` will execute on every call to `requestAnimationFrame`.                                                                                                                                                   |
