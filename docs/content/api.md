...
<img src="/static/pngs/gravity_animation.png" alt="The useGravity animation in renature." style="height: 30rem; margin: 5rem 0;" />
...
const GravityBasic: React.FC = () => {
  const [props] = useGravity<HTMLDivElement>({
    from: { opacity: 0, transform: 'translateX(0px) scale(0)' },
    to: { opacity: 1, transform: 'translateX(100px) scale(1)' },
    config: {
      moverMass: 10000,
...
<img src="/static/pngs/friction_animation.png" alt="The useFriction animation in renature." style="height: 36rem; margin: 5rem 0;" />
