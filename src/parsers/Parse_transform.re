  ...
  | Some(t) =>
    ...
    |> Js.Array.every(c => {
         ...
         | Some(cap) =>
           ...
           let isTransformProperty =
             switch (cap->transformPropertiesFromJs) {
             | Some(_) => true
             | None => false
             };
...
let testTransforms = val_ => {
  let transforms = Js.String.match(transformsRe, val_);
...
let parseTransform = val_ => {
  ...
  let t =
    ref({transform: Js.Nullable.null, transformProperty: Js.Nullable.null});
