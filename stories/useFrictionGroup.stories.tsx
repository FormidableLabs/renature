...
export const FrictionGroupSVG: FC = () => {
  ...
  const getPathLengthForIndex = useCallback<(i: number) => number>(
    (i) => {
      switch (i) {
        case 0:
          return pathLengths.inner;
        case 1:
          return pathLengths.outer;
        case 2:
          return pathLengths.circle;
        default:
          return 0;
      }
    },
  ...
  useLayoutEffect(() => {
    const derivedPathLengths = ['inner', 'outer', 'circle'].reduce(
      (acc, id) => {
        const element = document.querySelector<SVGPathElement>(`#${id}`);

        if (element) {
          return {
            ...acc,
            [id]: element.getTotalLength(),
          };
        }
    ...
    setPathLengths(derivedPathLengths);
  }, []);

  const [nodes] = useFrictionGroup<SVGPathElement>(3, (i) => ({
    from: {
      strokeDashoffset: 0,
    },
    to: {
  ...
  return (
    ...
    >
      ...
      <path
        ...
        id="inner"
        strokeDasharray={pathLengths.inner}
        {...nodes[0]}
      />
      <path
        ...
        className="react-logo"
        id="outer"
        strokeDasharray={pathLengths.outer}
        {...nodes[1]}
      />
      <path
